...
'''
    ...
    def test_ready_generation_stays_visible_during_file_refresh(self):
        ...
        self.assertIn("presentationHold.restart()", open_body)
        refresh_body = _balanced_body(cockpit, "id: presentationHold")
        self.assertIn("statusFile.reload()", refresh_body)
        self.assertIn("installCompletionFile.reload()", refresh_body)
        self.assertNotIn("statusResolved = false", refresh_body)
        self.assertNotIn("installCompletionResolved = false", refresh_body)
