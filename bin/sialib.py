...
# The thought-page, recovery and legacy-replay implementation lives in the
# bounded child module `siathought` (see its docstring and
# docs/ARCHITECTURE.md).  Two pieces of the lane stayed here deliberately: the
# ctypes block above, because a child executing ctypes.CDLL at import would
# build one libc handle per test alias, and the two context managers below,
# because invoke()'s bind ends when the call returns while a context
# manager's body runs later at __enter__ — so a context manager cannot be a
# delegate.  Both are reached from the child through bind().
...
def _append_bench_trend_once(record, receipt_id):
    ...
    # window before it can strand a durable dream-unit receipt at the file's
    # row and byte bounds.
