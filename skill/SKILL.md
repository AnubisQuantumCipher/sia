...
a git-versioned corpus indexed by gbrain with local embeddings. This session is
a client of that memory.
...
sia status                # readiness, chains, source summaries
...
- New persisted origin metadata has exactly three classes: `[evidence]`,
  ...
  evidence. Judge-grade/ponder entries, take-proposal notifications, and
...
- Typed domain relations come from event/epoch evidence or an explicitly
  `[derived]` integrity/healing/crash/refusal generated entry. Model and
  legacy-unlabeled entries may link pages only with generic `mentions`.
- Every `sia ask` answer ends with a truth-boundary line (sense
  ...
  user; never present an answer over stale SIA state as current.
...
- SIA's MCP server also exposes byte-bounded `sia://status`, `sia://thoughts`,
  ...
  `thoughts` and `cortex` are compatibility URI names for generated entries
  and the root page.
